...
fn create_random_sparse_matrix(nrows: usize, ncols: usize, density: f64) -> ArrayData {
    ...
    ArrayData::CsrMatrix(anndata::data::DynCsrMatrix::F64(csr))
}
...
fn test_matrix_data_integrity() -> anyhow::Result<()> {
    ...
    let csr = CsrMatrix::from(&coo);
    let matrix = ArrayData::CsrMatrix(anndata::data::DynCsrMatrix::F64(csr));
...
fn test_subset_value_integrity() -> anyhow::Result<()> {
    ...
    coo.push(0, 0, 10.0); // cell_0, gene_0 = 10.0
    coo.push(0, 2, 20.0); // cell_0, gene_2 = 20.0
    coo.push(1, 1, 30.0); // cell_1, gene_1 = 30.0
    coo.push(2, 0, 40.0); // cell_2, gene_0 = 40.0
    coo.push(2, 2, 50.0); // cell_2, gene_2 = 50.0
    coo.push(3, 1, 60.0); // cell_3, gene_1 = 60.0
    ...
    // Check values directly from CSR format
    if let ArrayData::CsrMatrix(dyn_csr) = subset_matrix_data {
        if let anndata::data::DynCsrMatrix::F64(csr_matrix) = dyn_csr {
            // Access CSR components directly
            let (row_offsets, col_indices, values) = csr_matrix.csr_data();
            ...
            // Check dimensions
            assert_eq!(csr_matrix.nrows(), 2, "Should have 2 rows");
            assert_eq!(csr_matrix.ncols(), 2, "Should have 2 columns");

            // Check specific values by examining CSR structure:
            // Row 0 (cell_1): should be empty (no non-zero values)
            let row0_start = row_offsets[0];
            let row0_end = row_offsets[1];
            let row0_nnz = row0_end - row0_start;
            assert_eq!(
                row0_nnz, 0,
            ...
            let row1_end = row_offsets[2];
            let row1_nnz = row1_end - row1_start;
            assert_eq!(row1_nnz, 2, "cell_2 should have 2 non-zero values");

            // Check the actual values for row 1 (cell_2)
            if row1_nnz == 2 {
                let col0_idx = col_indices[row1_start];
                let col0_val = values[row1_start];
                let col1_idx = col_indices[row1_start + 1];
                let col1_val = values[row1_start + 1];
